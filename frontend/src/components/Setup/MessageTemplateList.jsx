...
const MessageTemplateList = () => {
  const { messageTemplates, setMessageTemplates } = useContext(EmailTemplateContext);
  const [editingTemplate, setEditingTemplate] = useState(null);

  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/api/message-templates/${id}`);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:3000/api/message-templates/${editingTemplate._id}`,
        editingTemplate
      );
      setMessageTemplates((prevTemplates) =>
