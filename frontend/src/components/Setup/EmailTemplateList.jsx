// src/components/EmailTemplateList.jsx
...
const EmailTemplateList = () => {
  const { templates, setTemplates } = useContext(EmailTemplateContext);
  const [editingTemplate, setEditingTemplate] = useState(null);

  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/api/email-templates/${id}`);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:3000/api/email-templates/${editingTemplate._id}`,
        editingTemplate
      );
      setTemplates((prevTemplates) =>
